...
## June 9, 2026
- **Block-5 Business Cycle & Rotation implementation**
  Implemented Block 5 (Business Cycle & Rotation) end-to-end with six indicators: ISM Manufacturing PMI (25% weight), LEI YoY (25%), High Yield Spread (25%), Small vs Large (10%), Growth vs Value (10%), and IPO Activity (5%). Block 5 weight is 25% of total Macro Pulse Score; five-tier regime map ranges from "Recession / Contraction" to "Early Cycle / Recovery Boom" with "Healthy Expansion" in between. High Yield Spread sourced from FRED (`BAMLH0A0HYM2`, OAS converted ×100 to bps, scored descending); the other five are manual inputs bulk-seeded via a new `seed-block5-manual.ts` script (5,573 rows) and surfaced on `/admin/manual-inputs`. LEI bands a pre-computed `lei_yoy` column to sidestep the 2016 Conference Board rebase. Created `lib/scoring/blocks/business-cycle/` (block index + six scorers), extended the FRED series catalog and `BlockKey` union with `business_cycle`, registered the block in the scoring registry, and removed it from `PLANNED_BLOCK_NAMES` in the dashboard adapter. Research doc added at `Block5_Research--business_cycle.md`. Verified live on 2026-05-01: all six indicators match the validated seeded scores (block avg 3.85 → score 15 → Healthy Expansion).
  During verification, the Small vs Large (5.3) and Growth vs Value (5.4) scorers were pivoted from recomputing the 90-day price-ratio change in code to banding a pre-computed `ratio_90d_pct` column seeded under `IWM_SPY_RATIO_90D_PCT` / `SPYG_SPYV_RATIO_90D_PCT`. The in-code recompute diverged ~11% from the validated scores because the seeded raw ratio has mixed weekly/daily cadence with forward-fill, while the upstream `ratio_90d_pct` is derived from continuous daily prices. The transient `pctChangeOverWindow` helper was removed.

## June 15, 2026
- **Block-6 Political & Narrative Risk implementation**
  Implemented Block 6 (Political & Narrative Risk) end-to-end with six indicators: Geopolitical Risk (40% weight, `GPR_MONTHLY`, manual pre-scored 1–5), US Policy Uncertainty (25%, `USEPUINDXM`, FRED raw index scored descending), China–US Tension (10%, `EPU_CHINA`, manual pre-scored), Sanctions Activity (10%, `SANCTIONS_ACTIVITY`, manual event default 3), Global Easing (10%, `GLOBAL_EASING`, manual pre-scored), and Media Fear (5%, `GDELT_FEAR_TONE`, manual raw tone scored descending). Block 6 acts as a tail-risk overlay weighted 5% of total Macro Pulse Score; five-tier regime map ranges from "ACUTE EXTERNAL CRISIS" (1.0–1.8, risk-off bias justified) through "DEFAULT / BACKGROUND" (2.6–3.4, the most common state) to "COORDINATED TAILWIND" (4.3–5.0, synchronised global easing + calm geopolitics). US Policy Uncertainty sourced from FRED; the other five are manual inputs bulk-seeded via a new `seed-block6-manual.ts` script. Created `lib/scoring/blocks/political-narrative/` (block index + six scorers), extended the FRED series catalog and registered the block in the scoring registry, and removed it from `PLANNED_BLOCK_NAMES` in the dashboard adapter. Research doc added at `Block6_Research--political_narrative.md`.

- **Refactor: Tailwind class cleanup across admin components**
  Tidied Tailwind utility classes across ten admin components — coverage table, data-coverage / fetch-status / latest-snapshot cards, login/logout buttons, login dialog, block sidebar, block summary, and the admin sidebar. Simplified `block-sidebar` markup. No behavioral changes.
